...
# In your model's forward method:
    def forward(self, x):
        # x should have shape [batch_size, seq_len, features]
        # Where features include timeframe indicator
        
        # Split by timeframe
        timeframe_masks = {
            tf: (x[:,:,-1] == timeframe_idx)  # Assuming last feature is timeframe
            for tf, timeframe_idx in self.timeframe_indices.items()
        }
        
        outputs = {}
        for tf, mask in timeframe_masks.items():
            if mask.any():
                x_tf = x[mask]
                # Process each timeframe separately
                outputs[tf] = self.process_single_timeframe(x_tf)
