...
class TokenSpecificNormalizer(Normalizer):
    """Normalizer that scales each token's features independently"""
    def normalize(self, df):
        if not isinstance(df.index, pd.MultiIndex) or 'token' not in df.index.names:
            return super().normalize(df)
            
        if self.norm_type == "standardization":
            grouped = df.groupby(level='token')
            self.mean = grouped.transform('mean')
            self.std = grouped.transform('std')
            return (df - self.mean) / (self.std + np.finfo(float).eps)
            
        elif self.norm_type == "minmax":
            grouped = df.groupby(level='token')
            self.min_val = grouped.transform('min')
            self.max_val = grouped.transform('max')
            return (df - self.min_val) / (self.max_val - self.min_val + np.finfo(float).eps)
...
# Add to existing data_factory
from datasets.meme_coin_data import MemeCoinRegressionData

data_factory = {
    'weld': WeldData,
    'tsra': TSRegressionArchive,
    'pmu': PMUData,
    'meme_coin': MemeCoinRegressionData,  # Add this line
}